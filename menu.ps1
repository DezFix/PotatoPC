...
function Build-UpdatesPanel {
    ...
    foreach ($line in $lines) {
        ...
        if ($parts.Count -ge 4) {
            ...
            # Пропускаем строку-заголовок winget (Name / Id / Version / Available)
            if ($name -eq 'Name' -and $id -eq 'Id') { continue }

            # Пропускаем пакеты где версия не определена
            # winget пишет источник (winget/msstore) вместо версии если не знает текущую
            if ($version -match '^(winget|msstore|Unknown)$') { continue }
            if ($newVersion -match '^(winget|msstore|Unknown)$') { continue }

            # Пропускаем если Id выглядит как версия (артефакт парсинга)
    ...
    # Закреплённый заголовок колонок
