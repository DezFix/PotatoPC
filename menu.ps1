...
[void]$checkUpdatesBtn.Add_Click({
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        ...
        # Изменено: Удалена опция "Открыть Microsoft Store"
        ...
        if ($installWingetPrompt -eq "Открыть ссылку в браузере") {
            Write-Host "Попытка открыть ссылку в браузере: https://apps.microsoft.com/detail/9nblggh4nns1?hl=ru-RU&gl=UA" -ForegroundColor Yellow
            try {
                ...
                Write-Host "Ссылка в браузере должна быть открыта." -ForegroundColor Green
            } catch {
                Write-Host "Ошибка при открытии ссылки в браузере: $($_.Exception.Message)" -ForegroundColor Red
            }
