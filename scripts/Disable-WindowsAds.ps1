...
# Отключение рекламного идентификатора (Advertising ID)...
...
# Отключение рекомендаций и рекламы в меню Пуск...
...
# Отключение рекламы на экране блокировки...
...
# Отключение рекламы и рекомендаций в Проводнике (Explorer)...
...
# Отключение рекламы в приложении Параметры (Settings)...
...
# Отключение рекламного контента в настройках
...
# Отключение предлагаемых уведомлений
...
# Отключение Windows Welcome Experience
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Name "OemPreInstalledAppsEnabled" -Value 0 -ErrorAction SilentlyContinue
...
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Name "SubscribedContent-314563Enabled" -Value 0 -ErrorAction SilentlyContinue
...
# Отключение "Finish setting up your device"
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Name "SubscribedContent-338393Enabled" -Value 0 -ErrorAction SilentlyContinue
...
# Дополнительные настройки для полного отключения рекламы
...
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Name "FeatureManagementEnabled" -Value 0 -ErrorAction SilentlyContinue
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Name "RemediationRequired" -Value 0 -ErrorAction SilentlyContinue
...
# Отключение таргетированного опыта и веб-поиска в Пуске...
...
# Ограничение телеметрии (уровень 'Базовый'), чтобы уменьшить сбор данных для рекламы...
Set-RegValue -Path "HKLM:\Software\Policies\Microsoft\Windows\DataCollection" -Name "AllowTelemetry" -Value 1

# Отключение автоматического обновления приложений из Store (опционально, для контроля)...
...
Write-Host "Очистка кэша рекомендательных служб..." -ForegroundColor Cyan
...
Write-Host "Перезапуск Проводника Windows для применения настроек..." -ForegroundColor Cyan
Stop-Process -Name explorer -Force -ErrorAction SilentlyContinue
Start-Sleep -Seconds 2
Start-Process explorer.exe

Write-Host "Готово! Реклама и навязчивые рекомендации Windows отключены." -ForegroundColor Green
