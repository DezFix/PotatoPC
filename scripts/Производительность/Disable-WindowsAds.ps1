# NAME: Нет рекламе и слежке
# DESC: Комплексно отключает рекламу, рекомендации, автоустановку мусорных приложений и сбор данных для таргетинга
# TAGS: 1
# ICON: 🛡️
# RECOMMENDED: true
...
Write-Host "🚫 2. Отключение рекомендаций и автоустановки приложений (Start Menu & Cloud Content)..." -ForegroundColor Yellow
...
Set-RegValue -Path $cdmPath -Name "SilentInstalledAppsEnabled" -Value 0 # Запрет автоустановки "мусорных" приложений
...
# Глубокие политики (из моего скрипта)
Set-RegValue -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CloudContent" -Name "DisableWindowsConsumerFeatures" -Value 1
Set-RegValue -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CloudContent" -Name "DisableThirdPartySuggestions" -Value 1
...
Write-Host "🚫 4. Отключение рекламы в Проводнике (Explorer)..." -ForegroundColor Yellow
...
Set-RegValue -Path $expPath -Name "LaunchTo" -Value 1 # Открывать "Этот компьютер" вместо "Быстрого доступа"
...
Write-Host "🚫 6. Отключение таргетированного опыта и веб-поиска..." -ForegroundColor Yellow
...
Write-Host "🚫 7. Отключение сбора данных ввода и языка (Privacy)..." -ForegroundColor Yellow
...
Write-Host "🚫 8. Ограничение телеметрии (уровень 'Базовый' для совместимости)..." -ForegroundColor Yellow
...
Write-Host "`n✅ Готово! Реклама, навязчивые рекомендации и слежка за вводом отключены." -ForegroundColor Green
