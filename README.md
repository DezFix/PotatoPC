...
(3 попытки по 120 секунд) скипается, очередь идёт дальше. Если сбоев 2+
и стоит галка перезагрузки — отчёт упадёт на рабочий стол и сам откроется
в блокноте после перезагрузки (один раз, через RunOnce).
